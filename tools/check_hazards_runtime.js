...
(async ()=>{
  ...
  const hazards = await page.evaluate(() => ({ len: (window.game && window.game.level && window.game.level.hazards) ? window.game.level.hazards.length : -1 }));
  console.log('hazards length', hazards.len);
  await browser.close();
})();